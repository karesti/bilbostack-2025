...
public class StatsEndpoint {
   ...
   public void onOpen(Session session) {
      ...
      Log.info("Stats service socket opened");
      calculateStats();
   }
   ...
   private boolean cachesNotReady() {
      ...
      if (!remoteCacheManager.getCacheNames().contains(Shot.PLAYERS_SHOTS)) {
         Log.warn(String.format("%s cache does not exit", Shot.PLAYERS_SHOTS));
         return true;
      }
   ...
   public void onClose(Session session) {
      ...
      Log.info("Stats Service session has been closed");
   }

   @OnError
   public void onError(Session session, Throwable throwable) {
      sessions.remove(session.getId());
      Log.error("Stats Service session error", throwable);
   }
}
